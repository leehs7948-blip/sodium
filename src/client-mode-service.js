...
export class ClientModeService {
  ...
  async emergencyStop() {
    ...
    await Promise.all([...this.runtimes.values()].map(async (runtime) => {
      try { await runtime.stop(); } catch (error) { this.logger.log({ event: 'runtime_stop_failed', botId: runtime.botId, reason: error.message }); }
    }));
  ...
  async reconnectBot(botId) {
    ...
    if (!runtime) throw new Error(`Unknown botId: ${botId}`);
    await runtime.stop();
    await runtime.connect();
    return true;
  }
  ...
  async reconnectAll() {
    await Promise.all(BOT_IDS.map((id) => this.reconnectBot(id)));
    return true;
  }
